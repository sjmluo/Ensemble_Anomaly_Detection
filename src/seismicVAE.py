...
def callback():
    ...
    return [plateau,logbar, tb_callback]

def compile_model(model):
    optimizer = tf.keras.optimizers.Adam(learning_rate=1e-3)
    losses = [tf.losses.mean_absolute_error]*11
    losses.append(tf.losses.binary_crossentropy)
    model.compile(optimizer, loss=losses, loss_weights=[1, 1, 1, 1, 1, 1, 1, 2, 2, 2, 2, 1])
...
if __name__ == "__main__":
    ...
    modeltests(inp, tar, None, model = vae, epochs = 150, name = '05', 
    description="""VAE([[4,4,4,4,4,4,4,8,8,8,8],[8,8,8,8,8,8,16,32,32,32,32]], [516, 256, 128], 64, 
        ...
        , dir = 'src/reports/seismic', verbose = 2,overall_kwargs = {'callbacks': callback})
